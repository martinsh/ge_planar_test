...
void mtex_image_refl(vec3 I, vec4 camerafac, vec3 texco, sampler2D ima, float lodbias, mat4 objectmatrix, mat4 viewmatrix, mat4 viewmatrixinverse, vec3 vp, vec3 vn, out float value, out vec4 color)
{
	vec3 viewdirection = vec3(viewmatrixinverse * vec4(vp, 0.0));
	vec3 normaldirection = normalize(viewmatrixinverse * vec4(vn, 0.0)).xyz;
